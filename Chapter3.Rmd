...
Of the variables, only famrel and absences were found to be statistically significant on 0.05 and 0 levels, respectively.  
...
* **Pstatus**: is connected to intercept and PstatusT as described above. Now the odds ratio for intercept is less than 1 meaning negative association with high alcohol use. On the other hand, PstatusT has positive association (greater than 1). But both have a large 95 % CI in which number 1 is included, so the statistical significance is not strong.
* **Medu**: the variable is very close to 1 (neither positive nor negative association) with 95 % CI including the number 1 so no statistical significance.
...
* **absences**: greater than 1 and 95 % CI interval entirely above 1. Positive association with strong statistical significance.
...
* Hypothesis 4: Higher absences implies high_use. The hypothesis hold. This is, in fact, the most statistically significant independent variable in the model.
